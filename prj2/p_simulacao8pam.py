...
imagem_pam = np.zeros(len(imagem_bits)//3)

constelacao_8pam = np.asarray([-7., -5., -3., -1., 1., 3., 5., 7.])
constelacao_8pam /= np.sqrt(np.sum(constelacao_8pam**2))

for i in range(len(imagem_bits)//3):
    simbolo = int(imagem_bits[3*i]*4 + imagem_bits[3*i+1]*2 + imagem_bits[3*i+2])
    imagem_pam[i] = constelacao_8pam[simbolo]
...
SNR = 10
sigma2 = 10**(-SNR/10)
w = np.sqrt(sigma2/2)*np.random.randn(len(sinal_rx))
...
x_casado = np.convolve(sinal_rx, filtro)
x_dwnsp = g*x_casado[l*k - 1: l*k - 1 + len(imagem_pam)*k:k]
x_detec = np.zeros(len(imagem_pam))

im_bits_r = np.zeros(3*len(imagem_pam))

for i in range(len(imagem_pam)):
    x_detec[i] = np.argwhere(constelacao_8pam == decide(x_dwnsp[i], constelacao_8pam)[0])
    simb_norm = x_detec[i]
    im_bits_r[3*i] = simb_norm//4
    im_bits_r[3*i + 1] = (simb_norm//2)%2
    im_bits_r[3*i + 2] = simb_norm % 2

imagem_r = np.zeros(len(imagem))

im_bits_r = np.concatenate((im_bits_r, np.zeros(2)))
